...
void Client::StatusCallback(void *baton, const char *path, svn_wc_status_t *status)
{
    ...
    auto entry = status->entry;
    auto vEntry = Object::New(isolate);

    auto vStatus = Object::New(isolate);
    vStatus->Set(String::NewFromUtf8(isolate, "entry"), vEntry);
    ...
    vStatus->Set(String::NewFromUtf8(isolate, "locked"), Boolean::New(isolate, status->locked));
...
void Client::Status(const FunctionCallbackInfo<Value> &args)
{
    ...
    auto error = svn_client_status(&result_rev, *String::Utf8Value(args[0]), &revision, StatusCallback, &array, true, false, false, false, client->context, client->pool);
    if (error != nullptr)
    {
